...
export default function Cart() {
  ...
  const buyNow = async () => {
    if (checkedPurchases.length > 0) {
      ...
      const isValid = checkedPurchases.every(
        (purchase) => purchase.buy_count > 0 && purchase.buy_count <= purchase.product.quantity
      )
        ...
        {extendedPurchases.length > 0 ? (
          ...
          <div className='text-center'>
            ...
            <div className='mt-5 text-center'>
              <Link
                to={path.home}
                className='bg-orange px-8 py-2 uppercase text-white transition-all hover:bg-opacity-80'
              >
                Mua ngay
              </Link>
            </div>
